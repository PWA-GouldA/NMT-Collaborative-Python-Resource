...
media_order: 'samelist.png,people.png,differentlist.png'
date: '13:01 31-01-2019'
taxonomy:
    category:
        - docs
visible: true
...
There's many things [we can do with strings](handy-stuff-strings.md) and some of these things also work with lists.
...
Lists have a few [useful methods](https://docs.python.org/3/tutorial/datastructures.html#more-on-lists).

Some of the most commonly used ones are _append_, _extend_ and _remove_.
* `append` adds an item to the end of a list, 
* `extend` adds multiple items from another list and 
...
If we need to remove all matching items we can use a simple while loop. We'll talk more about loops [in the next section](loop-da-loop).
...
With [strings](handy-stuff-strings.md) we did something to them and then set the result back to the same variable, like `message = message.strip()`. This just doesn't work right with most mutable things because they're designed to be changed in-place.
...
This is the same thing that happened way back when [we assigned print's return value to a variable](using-functions.md#return-values).
...
This can be confusing at first, but it's actually easy to explain. 

The problem with this code example is the `b = a` line. If we draw a picture of the variables it looks like this:

![Same list.](samelist.png)
...
![Different lists.](differentlist.png)
...
So, why the heck would we use tuples instead of lists? 

There are some cases when we don't want mutability, but there are also cases when Python programmers just like to use tuples. If you want to know more about this you can read [Ned Batchelder's blog post about this](http://nedbatchelder.com/blog/201608/lists_vs_tuples.html).
